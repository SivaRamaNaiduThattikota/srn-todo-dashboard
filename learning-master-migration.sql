-- ═══════════════════════════════════════════════════════════════════════
-- SRN Command Center — MASTER Learning Migration (v1 + v2 combined)
-- Run this single file in Supabase SQL Editor.
-- Safe to run multiple times — fully idempotent.
-- Replaces both learning-progress-migration.sql and learning-v2-migration.sql
-- ═══════════════════════════════════════════════════════════════════════

-- ───────────────────────────────────────────────────────────────────────
-- PART 1: learning_progress  (topic-level checkbox state)
-- ───────────────────────────────────────────────────────────────────────
...
CREATE POLICY "allow_all_learning_progress"
  ON learning_progress FOR ALL USING (true) WITH CHECK (true);


-- ───────────────────────────────────────────────────────────────────────
-- PART 2: learning_phases  (phase definitions — replaces hardcoded TSX)
-- ───────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS learning_phases (
  ...
  tracks        jsonb   NOT NULL DEFAULT '[]',  -- [{label, topics:[string]}]
  weeks         jsonb   NOT NULL DEFAULT '[]',  -- [{label, goals:[string]}]
  practice      jsonb   NOT NULL DEFAULT '[]',  -- [{title, problems:[string]}]
  created_at    timestamptz NOT NULL DEFAULT now(),
  updated_at    timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_learning_phases_sort
  ON learning_phases (sort_order);
...
CREATE TRIGGER trg_learning_phases_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION update_learning_phases_updated_at();

ALTER TABLE learning_phases ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "allow_all_learning_phases" ON learning_phases;
CREATE POLICY "allow_all_learning_phases"
  ON learning_phases FOR ALL USING (true) WITH CHECK (true);


-- ───────────────────────────────────────────────────────────────────────
-- PART 3: learning_week_progress  (week-level checkbox state)
-- ───────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_learning_week_progress_phase
  ON learning_week_progress (phase_id);
...
CREATE TRIGGER trg_learning_week_progress_updated_at
  BEFORE UPDATE ON learning_week_progress
  FOR EACH ROW EXECUTE FUNCTION update_learning_week_progress_updated_at();
...
CREATE POLICY "allow_all_learning_week_progress"
  ON learning_week_progress FOR ALL USING (true) WITH CHECK (true);


-- ───────────────────────────────────────────────────────────────────────
-- PART 4: Seed 6 default phases (idempotent — skips if already present)
-- ───────────────────────────────────────────────────────────────────────
...
-- Make sure serial doesn't collide with seeded IDs
