...
export interface Subtask {
  id: string; todo_id: string; title: string; is_done: boolean; created_at: string;
}

export interface ActivityLog {
  id: string; todo_id: string | null;
  action: "created" | "status_changed" | "completed" | "deleted";
  old_value: string | null; new_value: string | null; created_at: string;
}

export interface TaskTemplate {
  id: string; title: string; description: string; priority: TodoPriority;
  assigned_agent: string; recurrence: "daily" | "weekly" | "monthly" | null;
...
export interface FocusSession {
  id: string; todo_id: string | null; duration_minutes: number;
  completed: boolean; started_at: string; ended_at: string | null;
}

export interface WeeklyReview {
  id: string; week_start: string; tasks_completed: number;
  focus_minutes: number; streak_days: number; reflection: string;
  goals_next_week: string; created_at: string;
}
...
export async function fetchSubtasks(todoId: string) {
  ...
  if (error) throw error; return data as Subtask[];
}
...
export async function fetchTemplates() {
  ...
  if (error) throw error; return data as TaskTemplate[];
}
...
export async function createTodoFromTemplate(template: TaskTemplate) {
  const todo = await addTodo({ title: template.title, description: template.description, priority: template.priority, assigned_agent: template.assigned_agent, status: "pending" });
...
export async function fetchHabits() {
  const { data, error } = await supabase.from("daily_habits").select("*").eq("is_active", true).order("sort_order", { ascending: true });
  if (error) throw error; return data as DailyHabit[];
}
export async function addHabit(name: string, color: string = "#6ee7b7") {
  const { data, error } = await supabase.from("daily_habits").insert({ name, icon: name.charAt(0).toUpperCase(), color }).select().single();
  if (error) throw error; return data as DailyHabit;
}
export async function deleteHabit(id: string) {
  await supabase.from("daily_habits").update({ is_active: false }).eq("id", id);
}
export async function fetchHabitLogs(days: number = 90) {
  const since = new Date(); since.setDate(since.getDate() - days);
  const { data, error } = await supabase.from("habit_log").select("*").gte("completed_date", since.toISOString().slice(0, 10)).order("completed_date", { ascending: false });
  if (error) throw error; return data as HabitLog[];
}
export async function toggleHabitDay(habitId: string, date: string) {
  // Check if exists
  const { data } = await supabase.from("habit_log").select("id").eq("habit_id", habitId).eq("completed_date", date).single();
  if (data) {
    await supabase.from("habit_log").delete().eq("id", data.id);
    return false; // unchecked
  } else {
    await supabase.from("habit_log").insert({ habit_id: habitId, completed_date: date });
    return true; // checked
  }
}
...
// ── Focus Sessions ─────────────────────────────────────────
export async function startFocusSession(todoId: string | null, minutes: number = 25) {
  const { data, error } = await supabase.from("focus_sessions").insert({ todo_id: todoId, duration_minutes: minutes }).select().single();
  if (error) throw error; return data as FocusSession;
}
export async function completeFocusSession(id: string) {
  await supabase.from("focus_sessions").update({ completed: true, ended_at: new Date().toISOString() }).eq("id", id);
}
export async function fetchFocusSessions(days: number = 30) {
  const since = new Date(); since.setDate(since.getDate() - days);
  const { data, error } = await supabase.from("focus_sessions").select("*").gte("started_at", since.toISOString()).order("started_at", { ascending: false });
  if (error) throw error; return data as FocusSession[];
}

// ── Weekly Reviews ─────────────────────────────────────────
export async function fetchWeeklyReviews() {
  const { data, error } = await supabase.from("weekly_reviews").select("*").order("week_start", { ascending: false }).limit(12);
  if (error) throw error; return data as WeeklyReview[];
}
export async function saveWeeklyReview(review: Partial<WeeklyReview>) {
  const { data, error } = await supabase.from("weekly_reviews").upsert(review, { onConflict: "week_start" }).select().single();
  if (error) throw error; return data as WeeklyReview;
}
...
export async function fetchActivityLog(days: number = 30) {
  ...
  if (error) throw error; return data as ActivityLog[];
}
