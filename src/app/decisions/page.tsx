...
export default function DecisionsPage() {
  ...
  const handleEdit = (d: Decision) => {
    setEditingId(d.id); setDecision(d.decision); setReasoning(d.reasoning); setExpectedOutcome(d.expected_outcome); setCategory(d.category);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <header className="mb-5 animate-fade-in-up">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs font-mono mt-1" style={{ color: "var(--text-muted)" }}>{decisions.length} decisions tracked · {dueForReview.length} due for review</p>
          </div>
          {!showForm && <button onClick={() => { resetForm(); setShowForm(true); }} className="cc-btn cc-btn-accent px-4 py-2 text-xs font-medium rounded-2xl"><span style={{ position: "relative", zIndex: 3 }}>+ Log decision</span></button>}
        </div>
      </header>
      ...
      {showForm && (
        <div className="spatial p-5 mb-5 animate-slide-up">
          ...
          <div className="space-y-3">
            <input type="text" value={decision} onChange={(e) => setDecision(e.target.value)} placeholder="What did you decide? *" autoFocus
            ...
            <textarea value={reasoning} onChange={(e) => setReasoning(e.target.value)} placeholder="Why? What's the reasoning behind this decision?" rows={2}
              className="w-full rounded-2xl px-4 py-3 text-xs font-mono focus:outline-none resize-none"
              style={{ background: "var(--bg-input)", border: "0.5px solid var(--glass-border)", color: "var(--text-primary)" }} />
            <textarea value={expectedOutcome} onChange={(e) => setExpectedOutcome(e.target.value)} placeholder="What outcome do you expect from this decision?" rows={2}
              className="w-full rounded-2xl px-4 py-3 text-xs font-mono focus:outline-none resize-none"
              style={{ background: "var(--bg-input)", border: "0.5px solid var(--glass-border)", color: "var(--text-primary)" }} />
            ...
            <button onClick={handleSave} disabled={!decision.trim()} className="w-full py-3 text-xs font-medium rounded-2xl transition-all disabled:opacity-30"
      ...
      {loading ? (
        ...
        <div className="spatial p-10 text-center"><p className="text-sm font-medium mb-2" style={{ color: "var(--text-primary)" }}>No decisions logged</p><p className="text-xs font-mono" style={{ color: "var(--text-muted)" }}>Track important decisions and revisit them later</p></div>
      ) : (
        <div className="space-y-3">
          {decisions.map((d, i) => {
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 space-y-3 animate-fade-in" style={{ borderTop: "0.5px solid var(--glass-border)" }}>
                      ...
                      {d.status === "active" && (
                        <div>
                          ...
                          <div className="flex gap-1.5">
                            {[{ status: "validated" as const, label: "Validated (good decision)", color: "#6ee7b7" }, { status: "reversed" as const, label: "Reversed (changed mind)", color: "#f87171" }, { status: "reviewed" as const, label: "Reviewed (still thinking)", color: "#60a5fa" }].map((opt) => (
                              <button key={opt.status} onClick={(e) => { e.stopPropagation(); handleReview(d.id, opt.status, ""); }}
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); deleteDecision(d.id).then(reload); }} className="px-3 py-1.5 text-[10px] font-mono rounded-xl" style={{ color: "#f87171" }}>Delete</button>
                      </div>
                    </div>
                  )}
                </div>
