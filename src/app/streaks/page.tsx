...
export default function StreaksPage() {
  ...
  const handleTouchStart = useCallback((
    ...
    longPressTimer.current = setTimeout(() => {
      ...
      setTooltip({ habitId, dateStr, done, x, y });
  ...
  const handleTouchEnd = useCallback((
    ...
    // Long press already fired — don't toggle, just let the dismiss useEffect handle closing
    if (longPressFired.current) {
      longPressFired.current = false;
      e.preventDefault(); // prevent the click event that would follow
      return;
    }
    // Quick tap — toggle normally
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    // If finger moved more than 8px the user is scrolling — cancel long press
    if (dx > 8 || dy > 8) {
      clearTimeout(longPressTimer.current);
      longPressTimer.current = null;
      longPressFired.current = false;
  ...
  // Tooltip position — flip left if near right edge, flip up if near bottom
      ...
      {tooltip && (
        <div className="tooltip-pop" style={tooltipStyle}>
          <div style={{
            ...
            <p style={{ fontSize: "9px", color: "rgba(255,255,255,0.25)", margin: "4px 0 0", fontFamily: "monospace" }}>
              tap to toggle \u00b7 hold = info
            </p>
          </div>
        </div>
      )}
