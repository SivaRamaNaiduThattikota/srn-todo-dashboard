...
export default function StreaksPage() {
  ...
  const heatmapDays = useMemo(() => {
    return eachDayOfInterval({ start: subDays(new Date(), 83), end: new Date() });
  }, []);
  ...
  const streaks = useMemo(() => {
    ...
    habits.forEach((h) => {
      ...
      for (let i = 0; i < 365; i++) {
        ...
        if (logs.some((l) => l.habit_id === h.id && l.completed_date === dateStr)) {
  ...
  // Overall streak (all habits done)
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          ...
          {overallStreak > 0 && (
            <div className="text-3xl sm:text-4xl font-bold font-mono" style={{ color: "#fbbf24" }}>
              {overallStreak}
              <span className="text-xs font-normal ml-1" style={{ color: "var(--text-muted)" }}>days</span>
            </div>
          )}
        </div>
      </header>

      {/* Habit cards with today toggle */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 animate-fade-in-up" style={{ animationDelay: "40ms" }}>
        {habits.map((h) => {
          ...
          return (
            <div key={h.id} className="glass rounded-2xl p-4 hover-lift" style={{ borderColor: done ? `${h.color}40` : "var(--border-default)" }}>
            ...
            {heatmapDays.map((day) => {
              ...
              return (
                <button key={dateStr} onClick={() => handleToggle(h.id, dateStr)}
                  ...
                  title={`${dateStr} — ${done ? "done" : "not done"}`} />
              );
            })}
          </div>
          <div className="flex justify-between mt-2">
            <span className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>12 weeks ago</span>
