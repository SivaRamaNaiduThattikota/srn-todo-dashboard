...
import { addTodo, updateTodo, deleteTodo, type TodoStatus, type TodoPriority, type Todo } from "@/lib/supabase";
...
export default function DashboardPage() {
  ...
  const handleAdd = async (data: { title: string; description: string; priority: TodoPriority; assigned_agent: string; due_date: string | null }) => {
  ...
  // CSV export
  const exportCSV = () => {
    const headers = "id,title,status,priority,assigned_agent,due_date,created_at,updated_at\n";
    const rows = todos.map((t) => `${t.id},"${t.title}",${t.status},${t.priority},${t.assigned_agent},${t.due_date || ""},${t.created_at},${t.updated_at}`).join("\n");
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-5 sm:py-8 sm:px-6 lg:px-8">
      ...
      <header className="mb-5 sm:mb-8 animate-fade-in-up">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1.5 text-[10px] font-mono px-2 py-0.5 rounded-full skeuo-raised" style={{ color: "var(--accent)", background: "var(--accent-muted)", border: "1px solid hsla(var(--accent-h), var(--accent-s), var(--accent-l), 0.2)" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-1">
              <button onClick={exportCSV} className="px-2 py-1.5 text-[10px] font-mono rounded-lg transition-all" style={{ color: "var(--text-muted)", background: "var(--bg-card)" }} title="Export CSV">CSV</button>
              <button onClick={exportJSON} className="px-2 py-1.5 text-[10px] font-mono rounded-lg transition-all" style={{ color: "var(--text-muted)", background: "var(--bg-card)" }} title="Export JSON">JSON</button>
            </div>
            <button onClick={() => setShowAdd(true)} className="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium liquid-glass rounded-xl hover-lift cursor-pointer" style={{ color: "var(--accent)" }}>
      ...
      {selected.size > 0 && (
        <div className="glass rounded-xl px-4 py-2.5 mb-4 flex items-center justify-between animate-fade-in-up">
          ...
          <div className="flex gap-2">
            <button onClick={bulkMarkDone} className="px-3 py-1 text-xs font-mono rounded-lg" style={{ background: "rgba(110,231,183,0.1)", color: "#6ee7b7" }}>Mark done</button>
            <button onClick={bulkDelete} className="px-3 py-1 text-xs font-mono rounded-lg" style={{ background: "rgba(248,113,113,0.1)", color: "#f87171" }}>Delete</button>
            <button onClick={() => setSelected(new Set())} className="px-3 py-1 text-xs font-mono rounded-lg" style={{ color: "var(--text-muted)" }}>Clear</button>
          </div>
        </div>
      )}
          ...
          <div className="flex gap-0.5 sm:gap-1 glass rounded-xl p-1 overflow-x-auto">
            {STATUS_FILTERS.map((s) => (
              <button key={s} onClick={() => setFilter(s)}
                ...
                {FILTER_LABELS[s]}
                <span className="text-[9px] font-mono px-1 rounded" style={{ background: filter === s ? "var(--bg-input)" : "transparent", color: "var(--text-muted)" }}>
                  {counts[s]}
                </span>
              </button>
            ))}
      ...
      {loading ? (
        ...
        <div className="glass rounded-2xl p-4 animate-fade-in-up" style={{ borderColor: "rgba(248,113,113,0.2)" }}>
          <span className="text-xs font-mono" style={{ color: "#f87171" }}>Connection error: {error}</span>
        </div>
      ) : filtered.length === 0 ? (
        /* ─── Empty State ─── */
        <div className="text-center py-16 sm:py-20 animate-fade-in">
          <div className="glass-heavy rounded-2xl inline-block px-8 sm:px-12 py-8 sm:py-10">
            ...
            {todos.length === 0 && (
              <button onClick={() => setShowAdd(true)} className="px-4 py-2 text-xs font-medium rounded-xl" style={{ background: "var(--accent)", color: "#0a0a0b" }}>
                Create first task
              </button>
            )}
          </div>
