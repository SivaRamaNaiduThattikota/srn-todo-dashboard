...
import {
  ...
  fetchTodos, fetchTemplates, createTodoFromTemplate,
...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
function statusColor(s: TodoStatus) {
...
function priorityColor(p: TodoPriority) {
...
function TodoModal({ todo, onSave, onClose }: ModalProps) {
  ...
  const [agent, setAgent]           = useState(todo?.assigned_agent ?? "srn");
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-xl rounded-t-[28px] sm:rounded-[24px] flex flex-col animate-slide-up"
        style={{
        ...
        <div className="flex gap-1 px-5 pt-3 flex-shrink-0">
          {(["basic", "details"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab === t ? "var(--accent-muted)" : "transparent", color: tab === t ? "var(--accent)" : "var(--text-muted)", border: `0.5px solid ${tab === t ? "var(--accent-dim)" : "transparent"}` }}>
              {t}
            </button>
          ))}
        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                  {STATUS_OPTIONS.map((s) => (
                    <button key={s.value} onClick={() => setStatus(s.value)}
                      className="flex items-center gap-1.5 px-2.5 py-2 rounded-[11px] text-[11px] font-mono transition-all"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                  {PRIORITY_OPTIONS.map((p) => (
                    <button key={p.value} onClick={() => setPriority(p.value)}
                      className="px-2.5 py-2 rounded-[11px] text-[11px] font-mono capitalize transition-all"
          ...
          {tab === "details" && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[10px] font-mono uppercase tracking-wider block mb-1.5" style={{ color: "var(--text-muted)" }}>Est. minutes</label>
              ...
              <div>
                <label className="text-[10px] font-mono uppercase tracking-wider block mb-1.5" style={{ color: "var(--text-muted)" }}>Tags (comma separated)</label>
                <input value={tags} onChange={(e) => setTags(e.target.value)}
                  placeholder="e.g. python, ml, urgent"
                  className="w-full rounded-[14px] px-3 py-2.5 text-xs font-mono focus:outline-none"
                  style={{ background: "var(--bg-input)", border: "0.5px solid var(--glass-border)", color: "var(--text-primary)" }} />
              </div>
            </>
...
export default function TasksPage() {
  ...
  const handleUseTemplate = async (tmpl: TaskTemplate) => {
    ...
    window.dispatchEvent(new CustomEvent("srn:toast", { detail: { message: `Created from template: ${tmpl.title}`, type: "success" } }));
  };
  ...
  const exportCSV = () => {
    ...
    filtered.forEach((t) =>
      rows.push([t.title, t.status, t.priority, t.category, t.assigned_agent, t.due_date ?? "", (t.tags ?? []).join("|")])
    );
        ...
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <div>
            ...
            <p className="text-xs font-mono mt-0.5" style={{ color: "var(--text-muted)" }}>
              {done}/{total} done
              {overdue > 0 && <span style={{ color: "#ff6b6b" }}> · {overdue} overdue</span>}
            </p>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap">
            ...
            <button onClick={exportCSV} className="cc-btn px-3 py-2 text-xs hidden sm:flex"
              style={{ color: "var(--cc-text-muted)" }} title="Export CSV">
            ...
            <button onClick={exportJSON} className="cc-btn px-3 py-2 text-xs hidden sm:flex"
              style={{ color: "var(--cc-text-muted)" }} title="Export JSON">
        ...
        <div className="mb-3 liquid-glass rounded-[16px] px-4 py-3 flex items-center gap-3 flex-wrap animate-fade-in">
          ...
          <div className="flex gap-1.5 flex-wrap ml-auto">
            ...
            <button onClick={handleBulkDelete}
              className="px-2.5 py-1.5 rounded-xl text-[10px] font-mono"
              style={{ background: "rgba(255,107,107,0.12)", color: "#ff6b6b", border: "0.5px solid rgba(255,107,107,0.28)" }}>
              Delete
            </button>
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: isSelected ? "0.5px solid var(--accent)" : undefined,
                  opacity: todo.status === "done" ? 0.72 : 1,
