...
import {
  ...
  fetchTemplates, createTodoFromTemplate, fetchFocusSessions,
  type Todo, type TodoStatus, type TodoPriority, type TodoCategory,
  type ResourceLink, type TaskTemplate, type FocusSession, type ChecklistItem,
} from "@/lib/supabase";
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((todo, idx) => {
            ...
            const due = formatDate(todo.due_date, todo.status);
            const sc  = STATUS_OPTIONS.find((s) => s.value === todo.status)!;
            const focusData      = focusSessions.filter((s) => s.completed && s.todo_id === todo.id);
            const focusedMins    = focusData.reduce((sum, s) => sum + s.duration_minutes, 0);
