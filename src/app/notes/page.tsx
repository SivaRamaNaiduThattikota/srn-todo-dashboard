...
const PRESET_TAGS = [
  "python", "sql", "ml", "deep-learning", "llm", "nlp",
  "interview", "system-design", "dsa", "cloud", "mlops",
  "stats", "project", "concept",
];
...
function HighlightText({ text, query, maxLen = 120 }: { text: string; query: string; maxLen?: number }) {
  ...
  const qInSnip  = snipLow.indexOf(qLower);
...
export default function NotesPage() {
  ...
  /* Delete undo */
  const [undoState, setUndoState]       = useState<NoteUndoState | null>(null);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const allTags = [...tags, ...(customTag.trim() ? customTag.split(",").map((t) => t.trim()).filter(Boolean) : [])];
      if (editingId) await updateNote(editingId, { title: title.trim(), content: content.trim(), tags: allTags });
      else           await addNote({ title: title.trim(), content: content.trim(), tags: allTags });
      setNotes(await fetchNotes()); resetForm();
  ...
  const handleUndo = () => {
    ...
    if (undoProgressRef.current) clearInterval(undoProgressRef.current);
    /* Restore the note visually */
    setNotes((prev) => {
      if (prev.find((n) => n.id === undoState.note.id)) return prev;
      return [undoState.note, ...prev];
    });
    setUndoState(null);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto pb-32 md:pb-10">
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-xs font-mono mt-0.5" style={{ color: "var(--text-muted)" }}>
              {notes.length} notes · {filtered.length !== notes.length ? `${filtered.length} shown` : "all"}
            </p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {!showAdd && (
              <button onClick={() => { resetForm(); setShowAdd(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
      ...
      <div className="relative mb-3 animate-fade-in-up" style={{ animationDelay: "15ms" }}>
        <div
          ...
          style={{
            ...
            border: `0.5px solid ${search ? "var(--accent)" : "var(--cc-tile-border)"}`,
            backdropFilter: "blur(20px) saturate(1.8)",
            boxShadow: search ? `0 0 0 3px var(--accent-muted), var(--cc-outer-shadow)` : "var(--cc-outer-shadow)",
          }}
        >
          ...
          {search && (
            <span className="text-[9px] font-mono flex-shrink-0 px-2 py-0.5 rounded-lg"
              style={{ background: filtered.length > 0 ? "var(--accent-muted)" : "rgba(248,65,65,0.12)", color: filtered.length > 0 ? "var(--accent)" : "#f87171" }}>
      ...
      <div
        className="flex gap-1.5 mb-4 animate-fade-in-up pb-0.5"
        style={{ animationDelay: "25ms", overflowX: "auto", WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}
      >
        {/* All button */}
        <button
          ...
          style={{
            padding: "7px 12px",
            background: !filterTag ? "var(--accent-muted)" : "var(--glass-fill)",
            border:     `0.5px solid ${!filterTag ? "var(--accent-dim)" : "var(--glass-border)"}`,
            color:      !filterTag ? "var(--accent)" : "var(--cc-text-muted)",
            backdropFilter: "blur(12px)",
          }}
        >
          All
          <span style={{ opacity: 0.65 }}>({notes.length})</span>
        </button>
          ...
          return (
            <button key={t}
              ...
              style={{
                ...
                border:     `0.5px solid ${active ? "var(--accent-dim)" : "var(--glass-border)"}`,
            ...
            >
              {t}
              <span style={{ opacity: 0.65 }}>({tagCounts[t] || 0})</span>
            </button>
          );
        })}
      </div>
          ...
          <div className="space-y-3">
            ...
            {content && (
              <p className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>
                {readStats(content).words} words · ~{readStats(content).mins} min read
              </p>
            )}
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {PRESET_TAGS.map((t) => (
                  <button key={t} onClick={() => toggleTag(t)}
                    className="px-2.5 py-1.5 text-[10px] font-mono rounded-xl transition-all"
                    style={{
                      background: tags.includes(t) ? "var(--accent-muted)" : "var(--bg-input)",
                      color:      tags.includes(t) ? "var(--accent)"       : "var(--text-muted)",
                      border:     `0.5px solid ${tags.includes(t) ? "var(--accent-dim)" : "var(--border-default)"}`,
                    }}>
                    {t}
                  </button>
                ))}
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={handleSave} disabled={!title.trim() || saving}
                className="flex-1 py-3 text-xs font-medium rounded-2xl disabled:opacity-30 transition-all"
      ...
      {loading ? (
        ...
        <div className="text-center py-16 liquid-glass rounded-[22px] animate-fade-in">
          <p className="text-sm font-medium mb-1" style={{ color: "var(--text-primary)" }}>
            {notes.length === 0 ? "No notes yet" : search ? `No results for "${search}"` : "No notes match this tag"}
          </p>
          <p className="text-xs font-mono mb-4" style={{ color: "var(--text-muted)" }}>
            {notes.length === 0 ? "Save ML concepts, code snippets, and interview questions here" : "Try a different search or tag filter"}
          </p>
          {(search || filterTag) && (
            <button onClick={() => { setSearch(""); setFilterTag(""); }} className="px-4 py-2 text-xs font-medium rounded-xl" style={{ background: "var(--glass-fill-hover)", color: "var(--text-primary)", border: "0.5px solid var(--glass-border)" }}>
              Clear filters
            </button>
          )}
        </div>
            ...
            return (
              <div key={note.id}
                ...
                style={{
                  ...
                  borderColor: note.is_pinned ? "hsla(var(--accent-h),var(--accent-s),var(--accent-l),0.28)" : "var(--glass-border)",
              ...
              >
                {/* Pinned top stripe */}
                {note.is_pinned && (
                  <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: `linear-gradient(90deg, var(--accent) 0%, transparent 100%)` }} />
                )}
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex gap-1 flex-shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      style={{ color: note.is_pinned ? "var(--accent)" : "var(--cc-text-muted)", background: note.is_pinned ? "var(--accent-muted)" : "var(--glass-fill)" }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); startDeleteCountdown(note); }}
                      className="w-7 h-7 flex items-center justify-center rounded-lg transition-all"
                      style={{ color: "#f87171", background: "rgba(248,65,65,0.08)" }}
                      title="Delete"
                    >
                ...
                {/* Footer: tags + stats + date */}
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex gap-1 flex-wrap">
                    {note.tags?.slice(0, 3).map((t) => (
                      <span key={t} className="text-[8px] font-mono px-1.5 py-0.5 rounded-lg"
                        style={{ background: filterTag === t ? "var(--accent-muted)" : "var(--bg-input)", color: filterTag === t ? "var(--accent)" : "var(--text-muted)" }}>
                        {t}
                      </span>
                    ))}
            ...
            return (
              <div key={note.id}
                ...
                style={{
                  ...
                  borderColor: note.is_pinned ? "hsla(var(--accent-h),var(--accent-s),var(--accent-l),0.22)" : "var(--glass-border)",
                  opacity: isPendingDelete ? 0.4 : 1,
                  transition: "opacity 0.3s ease",
                }}
                onClick={() => handleEdit(note)}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {note.content && (
                    <p className="text-[10px] font-mono line-clamp-1" style={{ color: "var(--text-secondary)" }}>
                      <HighlightText text={note.content} query={search} maxLen={100} />
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
      ...
      {undoState && (
        <div className="fixed left-1/2 animate-slide-up z-50"
          style={{ transform: "translateX(-50%)", bottom: "calc(80px + env(safe-area-inset-bottom,0px) + 8px)", width: "calc(100vw - 32px)", maxWidth: "360px" }}>
