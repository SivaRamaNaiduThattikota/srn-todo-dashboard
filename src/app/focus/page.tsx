...
export default function FocusPage() {
  ...
  const dailyStats = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      return {
        date: dayStr, day: format(day, "EEE"), dayShort: format(day, "d"),
        minutes: totalMinutes, sessions: daySessions.length,
      };
    });
  }, [sessions]);
  ...
  const handleManualLog = async () => {
    ...
    try {
      // Build a timestamp for the chosen date + hour
              ...
              <div className="flex gap-1">
                {[25, 45, 60, 90, 120].map((d) => (
                  <button key={d}
                    ...
                    className="flex-1 py-2 text-[9px] font-mono rounded-[10px] transition-all"
                    style={{
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] font-mono" style={{ color: "var(--text-muted)" }}>
              Logging: <span style={{ color: "var(--accent)" }}>
                {manualCustomDur ? `${manualCustomDur}m` : `${manualDuration}m`}
              </span> on{" "}
                ...
                <div className="flex gap-2 justify-center">
                  {DURATIONS.map((d) => (
                    <button key={d} onClick={() => { setDuration(d); setTimeLeft(d * 60); }}
                      ...
                      style={{ color: duration === d ? "var(--accent)" : "var(--text-secondary)", minWidth: "52px" }}
