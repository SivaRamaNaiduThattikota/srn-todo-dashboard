...
/* 6 durations → renders as 3×2 grid on mobile, flex row on desktop */
  ...
  const dailyStats = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      const daySessions = sessions.filter(
        (s) => s.completed && format(new Date(s.started_at), "yyyy-MM-dd") === dayStr
      );
      const totalMinutes = daySessions.reduce((sum, s) => sum + s.duration_minutes, 0);
      return { date: dayStr, day: format(day, "EEE"), minutes: totalMinutes, sessions: daySessions.length };
    });
  }, [sessions]);
  ...
  const handleManualLog = async () => {
    ...
    try {
      ...
      const endedAt   = new Date(
        new Date(`${manualDate}T${manualHour}:00:00`).getTime() + mins * 60 * 1000
      ).toISOString();

      const { error } = await supabase.from("focus_sessions").insert({
        todo_id:          manualTodo || null,
      ...
      {showManual && (
        <div className="liquid-glass rounded-[22px] p-4 sm:p-5 mb-4 animate-slide-up">
          ...
          <p className="text-[10px] font-mono mb-4" style={{ color: "var(--text-muted)" }}>
            Forgot to start the timer? Add any session from the past here.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            ...
            {/* Duration preset — 3×2 grid on mobile */}
            <div>
              ...
              <div className="grid grid-cols-3 sm:flex gap-1">
                {[25, 45, 60, 90, 120, 180].map((d) => (
                  <button key={d}
                    onClick={() => { setManualDuration(d); setManualCustomDur(""); }}
                    className="py-2 text-[9px] font-mono rounded-[10px] transition-all"
                    style={{
                      background: manualDuration === d && !manualCustomDur ? "var(--accent-muted)" : "var(--bg-input)",
                      color:      manualDuration === d && !manualCustomDur ? "var(--accent)" : "var(--text-muted)",
                      border:     `0.5px solid ${manualDuration === d && !manualCustomDur ? "var(--accent-dim)" : "var(--glass-border)"}`,
                    }}>
                    {d >= 60 ? `${d / 60}h` : `${d}m`}
                  </button>
                ))}
            ...
            <div>
              ...
              <input type="number" min="1" max="480" value={manualCustomDur}
                onChange={(e) => setManualCustomDur(e.target.value)}
                placeholder="e.g. 110"
                className="w-full rounded-[12px] px-3 py-2 text-xs font-mono focus:outline-none"
                style={{ background: "var(--bg-input)", border: "0.5px solid var(--glass-border)", color: "var(--text-primary)" }} />
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] font-mono" style={{ color: "var(--text-muted)" }}>
              Logging: <span style={{ color: "var(--accent)" }}>
                {manualCustomDur ? `${manualCustomDur}m` : `${manualDuration >= 60 ? `${manualDuration / 60}h` : `${manualDuration}m`}`}
              </span> on{" "}
              <span style={{ color: "var(--text-secondary)" }}>
                {manualDate === format(new Date(), "yyyy-MM-dd")
                  ? "today"
                  : manualDate === format(subDays(new Date(), 1), "yyyy-MM-dd")
                  ? "yesterday"
                  : manualDate}
              </span>
              {" "}at{" "}
              <span style={{ color: "var(--text-secondary)" }}>
                {parseInt(manualHour) === 0 ? "12" : parseInt(manualHour) > 12 ? parseInt(manualHour) - 12 : parseInt(manualHour)}:00{" "}
                {parseInt(manualHour) < 12 ? "AM" : "PM"}
              </span>
            </p>
            <button onClick={handleManualLog} disabled={savingManual}
              className="cc-btn cc-btn-accent px-5 py-2 text-xs disabled:opacity-40">
              <span style={{ position: "relative", zIndex: 3 }}>
                {savingManual ? "Saving…" : "✓ Log session"}
              </span>
            </button>
          </div>
        </div>
          ...
          <div className="liquid-glass rounded-[24px] p-6 sm:p-8 text-center mb-4 animate-fade-in-up">
            ...
            <div className="relative inline-block mb-6">
              <svg width="180" height="180" viewBox="0 0 180 180" className="transform -rotate-90">
                <circle cx="90" cy="90" r="80" fill="none" strokeWidth="3" style={{ stroke: "var(--glass-border)" }} />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-4xl font-mono font-bold"
                  style={{ color: "var(--text-primary)", fontFamily: "-apple-system, 'SF Mono', monospace", letterSpacing: "-0.03em", textShadow: isRunning ? "0 0 20px var(--accent-glow)" : "none" }}>
                  {String(mins).padStart(2, "0")}:{String(secs).padStart(2, "0")}
                </span>
                <span className="text-[10px] font-mono mt-1.5 uppercase tracking-widest"
            ...
            {!isRunning ? (
              <div className="space-y-4">
                {/* Duration chips — 3×2 grid on mobile, flex row on sm+ */}
                <div className="grid grid-cols-3 sm:flex gap-2 justify-center">
                  {DURATIONS.map((d) => (
                    <button key={d} onClick={() => { setDuration(d); setTimeLeft(d * 60); }}
                      className="cc-chip py-2 text-xs relative z-10"
                      style={{ color: duration === d ? "var(--accent)" : "var(--text-secondary)", flex: "0 0 auto" }}
                ...
                {/* Start */}
                <button onClick={handleStart} className="cc-btn cc-btn-accent px-10 py-3 text-sm" style={{ minWidth: "140px" }}>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <button onClick={handleStop} className="cc-btn cc-btn-danger px-8 py-2.5 text-xs" style={{ minWidth: "110px" }}>
            ...
            {sessions.filter((s) => s.completed).length === 0 ? (
              ...
              <div className="space-y-1.5 max-h-[280px] overflow-y-auto">
                {sessions.filter((s) => s.completed).slice(0, 25).map((s) => {
                  ...
                  return (
                    <div key={s.id} className="flex items-center justify-between py-1.5 px-2.5 rounded-[11px]"
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        ...
                        <span className="text-[9px] font-mono" style={{ color: "var(--text-muted)" }}>{dateLabel} {timeLabel}</span>
                      </div>
                    </div>
                  );
                })}
