...
import { fetchActivityLog, fetchLearningStats, type ActivityLog } from "@/lib/supabase";
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            <Link href="/learning" className="block no-underline animate-fade-in-up">
              <div className="glass rounded-2xl p-4 sm:p-5 hover-lift">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  {[
                    { label: "Topics done",  value: learnStats.doneTopics,  total: learnStats.totalTopics, pct: learnPct,  color: "var(--accent)" },
                    { label: "Topics left",  value: learnStats.totalTopics - learnStats.doneTopics, total: null, pct: null, color: "var(--text-secondary)" },
                    { label: "Weeks done",   value: learnStats.doneWeeks,   total: learnStats.totalWeeks,  pct: weeksPct,  color: "#5ecf95" },
                    { label: "Weeks left",   value: learnStats.totalWeeks - learnStats.doneWeeks,  total: null, pct: null, color: "var(--text-secondary)" },
                  ].map((s) => (
          ...
          <div className="glass rounded-2xl p-4 sm:p-6 animate-fade-in-up" style={{ animationDelay: "80ms" }}>
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
              ...
              <div className="flex items-center gap-3 sm:gap-4 text-[10px] font-mono flex-wrap" style={{ color: "var(--text-muted)" }}>
                ...
                {velocityTrend !== 0 && <span style={{ color: velocityTrend > 0 ? "#6ee7b7" : "#f87171" }}>{velocityTrend > 0 ? "↑" : "↓"} {Math.abs(velocityTrend)}% vs last week</span>}
              </div>
            </div>
              ...
              {chartData.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex gap-0.5 items-end" style={{ height: "90px" }}>
                    <div className="flex-1 rounded-t-sm transition-all duration-500" style={{ height: `${Math.max(2, (d.created / maxChart) * 90)}px`, background: "#60a5fa", opacity: 0.7 }} />
                    <div className="flex-1 rounded-t-sm transition-all duration-500" style={{ height: `${Math.max(2, (d.completed / maxChart) * 90)}px`, background: "#6ee7b7", opacity: 0.8 }} />
                  </div>
          ...
          <div className="glass rounded-2xl p-4 sm:p-6 animate-fade-in-up" style={{ animationDelay: "240ms" }}>
            ...
            {actLoading ? (
              ...
              <p className="text-xs font-mono" style={{ color: "var(--text-muted)" }}>No activity yet. Start creating and completing tasks to see history here.</p>
            ) : (
