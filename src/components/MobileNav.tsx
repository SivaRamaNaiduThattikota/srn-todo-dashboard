...
export function MobileNav() {
  ...
  useEffect(() => {
    if (sheetOpen) setNavHidden(false);
  }, [sheetOpen]);
  ...
  return (
    <>
      {/* Bottom bar */}
      <nav className={`mobile-bottom-nav ${navHidden ? "nav-hidden" : "nav-visible"}`} style={{
        ...
        <div style={{
          ...
          >
            ...
            <span style={{
              color: (isMoreActive || sheetOpen) ? "var(--accent)" : "var(--cc-text)",
              display: "flex",
              marginTop: (isMoreActive || sheetOpen) ? "5px" : "0",
              transition: "all 0.22s var(--sp)",
            }}>
              {sheetOpen
      ...
      {sheetOpen && (
        <>
          ...
          <div style={{
            ...
            animation: "slideUp 0.3s var(--sp) both",
          }}>
            {/* Drag handle */}
              ...
              {MORE_ITEMS.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} style={{
                    ...
                    borderRadius: "16px",
                    background: isActive ? "var(--accent-muted)" : "var(--cc-glass-base)",
                    border: isActive ? "0.5px solid var(--accent-dim)" : "0.5px solid var(--cc-tile-border)",
                    boxShadow: isActive
                      ? "inset 0 1px 0 rgba(255,255,255,0.18), 0 0 16px var(--accent-muted)"
                      : "var(--cc-inner-shadow), 0 2px 8px rgba(0,0,0,0.10)",
                    textDecoration: "none",
                    position: "relative", overflow: "hidden",
                    transition: "all 0.2s var(--sp)",
                  }}>
                    ...
                    <span style={{ fontSize: "10px", fontWeight: 600, color: isActive ? "var(--accent)" : "var(--cc-text)", fontFamily: "-apple-system, SF Pro Display, sans-serif", letterSpacing: "-0.01em", textAlign: "center", lineHeight: 1.2, position: "relative", zIndex: 1 }}>
                      {item.label}
                    </span>
                  </Link>
                );
