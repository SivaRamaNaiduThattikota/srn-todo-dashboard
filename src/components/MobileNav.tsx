...
function TabItem({ href, label, iconKey, isActive }: {
  ...
  return (
    <Link href={href} style={{
      ...
      gap: "3px", padding: "4px", borderRadius: "12px",
      ...
      transition: "all 0.22s var(--sp)", minWidth: "52px",
...
export function MobileNav() {
  ...
  useEffect(() => {
    ...
    const onTouchMove  = (e: TouchEvent) => {
      ...
      if (dy > 10 && window.scrollY > 60) setNavHidden(true);
      if (dy < -6)                         setNavHidden(false);
    };
  ...
  useEffect(() => { if (sheetOpen) setNavHidden(false); },        [sheetOpen]);
    ...
    <>
      ...
      <nav
        ...
        style={{
          ...
          zIndex: 50,
          // Hide/show via transform — no layout reflow
          transform: navHidden
            ? "translateY(calc(100% + env(safe-area-inset-bottom, 0px)))"
            : "translateY(0)",
          transition: navHidden
            ? "transform 0.28s var(--ei), opacity 0.22s ease"
            : "transform 0.34s var(--sp), opacity 0.28s ease",
      ...
      >
        ...
        <div style={{
          ...
          {PRIMARY_TABS.map((item) => (
            <TabItem key={item.href} href={item.href} label={item.label} iconKey={item.iconKey} isActive={pathname === item.href} />
          ))}
          ...
          <button
            ...
            style={{
              flex: 1, display: "flex", flexDirection: "column",
              alignItems: "center", justifyContent: "center",
              gap: "3px", padding: "4px", borderRadius: "12px", minWidth: "52px",
              ...
              cursor: "pointer", transition: "all 0.22s var(--sp)", position: "relative",
            }}
          >
