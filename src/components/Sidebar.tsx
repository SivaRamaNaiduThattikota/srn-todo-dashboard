...
const NAV_ITEMS = [
  ...
  { href: "/projects", label: "Projects", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg> },
  ...
  { href: "/assistant", label: "AI", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M6 20v-2a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v2"/></svg> },
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-16 lg:w-56 z-40 flex flex-col py-5 transition-all duration-300"
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg transition-all duration-150 group"
              style={{
                background: isActive ? "var(--accent-muted)" : "transparent",
                color: isActive ? "var(--accent)" : "var(--text-muted)",
              }}>
              <span className="flex-shrink-0">{item.icon}</span>
              <span className="hidden lg:block text-[11px] font-medium" style={{ color: isActive ? "var(--text-primary)" : "var(--text-secondary)" }}>{item.label}</span>
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-2 lg:px-2.5 mt-auto">
        <Link href="/settings" className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg transition-all duration-150"
          ...
          <span className="hidden lg:block text-[11px] font-medium" style={{ color: pathname === "/settings" ? "var(--text-primary)" : "var(--text-secondary)" }}>Settings</span>
        </Link>
      </div>
    </aside>
  );
