...
// Resource type pills with icons — no native select needed
...
export function AddTodoModal({ onAdd, onClose, prefillDueDate }: Props) {
  ...
  // linkType: use pill buttons, no native select
  const [linkType, setLinkType]   = useState<ResourceLinkType>("article");
  // When "other" selected, user can type a custom label stored in link title prefix
  ...
  // Fix: handleSubmit now calls onClose() on success so modal actually closes
  const handleSubmit = async () => {
    ...
    try {
      ...
      onClose(); // ← Fix: close modal after successful add
    } catch (err: any) {
  ...
  // Fix: title is optional (auto-extracted from URL), URL is required
  // "other" type stores customTypeLabel as the type label
  const addLink = () => {
    setLinkError(null);
    if (!linkUrl.trim()) { setLinkError("URL is required"); return; }
    const url  = linkUrl.trim().startsWith("http") ? linkUrl.trim() : `https://${linkUrl.trim()}`;
    // Determine the actual type: for "other", we still save as "other" but title includes custom label
    const detectedType = detectLinkType(url);
    const finalType    = linkType === "article" ? detectedType : linkType; // respect user's manual choice
    const finalTitle   = linkTitle.trim() || extractTitle(url);
    // For "other" with custom label, prepend it to title so it shows nicely
    const displayTitle = (linkType === "other" && customTypeLabel.trim())
      ? `[${customTypeLabel.trim()}] ${finalTitle}`
      : finalTitle;

    setLinks((p) => [...p, { url, title: displayTitle, type: finalType }]);
    setLinkUrl(""); setLinkTitle(""); setLinkType("article"); setCustomTypeLabel(""); setLinkError(null);
  };
  ...
  // Estimated time display helper
  const estLabel = estimatedMins ? fmtMins(estimatedMins) : null;
  ...
  return (
    <>
      ...
      <div className="fixed z-[61] left-0 right-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center sm:px-4">
        <div className="relative w-full sm:max-w-[480px] animate-slide-up flex flex-col"
          style={{
          ...
          <style>{`@media(min-width:640px){.modal-inner{border-radius:28px!important;}}`}</style>
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-[11px] font-mono mt-0.5" style={{ color: "var(--text-muted)" }}>
                  ...
                  {estLabel && <span style={{ color: "var(--accent)" }}> · est {estLabel}</span>}
                </p>
              </div>
          ...
          {error && (
            <div className="mx-5 mb-2 px-3 py-2 rounded-[12px] text-xs font-mono flex-shrink-0"
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-2" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch", minHeight: 0 }}>
            ...
            {tab === "details" && (
              <div className="space-y-3 py-2">
                <FieldLabel label="Estimated time" hint="how long will this take?">
                  ...
                  {estimatedMins && (
                    <p className="text-[10px] font-mono mt-1.5" style={{ color: "var(--text-muted)" }}>
                      Set to <span style={{ color: "var(--accent)" }}>{fmtMins(estimatedMins)}</span>.
                      Focus sessions will be compared against this estimate in the expanded task card.
                    </p>
                  )}
                </FieldLabel>
            ...
            {tab === "resources" && (
              <div className="space-y-3 py-2">
                <p className="text-[11px] font-mono" style={{ color: "var(--text-muted)" }}>
                  Attach reference links — articles, videos, GitHub repos, docs. URL is required; title is auto-detected.
                </p>
                <div className="rounded-[16px] p-3 space-y-2.5" style={{ background: "var(--glass-fill)", border: "0.5px solid var(--glass-border)" }}>

                  {/* ── Type selector — pill buttons, NOT a native select ── */}
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wider mb-1.5" style={{ color: "var(--text-muted)" }}>Type</p>
                    <div className="flex gap-1.5 flex-wrap">
                      {LINK_TYPES.map((lt) => (
                        <button key={lt.value} type="button" onClick={() => { setLinkType(lt.value); setCustomTypeLabel(""); }}
                    ...
                    {/* When "other" is selected, show a custom type input */}
                    {linkType === "other" && (
                      <input type="text" value={customTypeLabel} onChange={(e) => setCustomTypeLabel(e.target.value)}
                        placeholder="Custom type label e.g. Paper, Blog, Forum…"
                        className="w-full focus:outline-none mt-2"
                        style={{ ...inputStyle, fontSize: "11px", padding: "8px 12px",
                          border: "0.5px solid #94a3b860", background: "rgba(148,163,184,0.10)" }} />
                    )}
                  </div>

                  {/* URL — required */}
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wider mb-1" style={{ color: "var(--text-muted)" }}>URL <span style={{ color: "var(--accent)" }}>*</span></p>
                    <input type="url" value={linkUrl}
                      onChange={(e) => { setLinkUrl(e.target.value); setLinkError(null); if (!linkTitle) setLinkType(detectLinkType(e.target.value)); }}
                      onKeyDown={(e) => { if (e.key==="Enter"){e.preventDefault();addLink();} }}
                      placeholder="https://..." className="w-full focus:outline-none"
                      style={{ ...inputStyle, fontSize: "12px", border: linkError ? "0.5px solid rgba(255,107,107,0.50)" : "0.5px solid var(--glass-border)" }} />
                    {linkError && <p className="text-[10px] font-mono mt-1" style={{ color: "#ff6b6b" }}>⚠ {linkError}</p>}
                  </div>

                  {/* Title — optional */}
                  <div>
                    <p className="text-[9px] font-mono uppercase tracking-wider mb-1" style={{ color: "var(--text-muted)" }}>Title <span style={{ color: "var(--text-tertiary)" }}>(optional — auto-detected)</span></p>
                  ...
                  <button type="button" onClick={addLink}
                    className="cc-btn cc-btn-accent w-full py-2.5 text-[12px]">
                ...
                {links.length > 0 ? (
                  <div className="space-y-2">
                    {links.map((link, i) => {
                      const lt = LINK_TYPES.find((x) => x.value === link.type) || LINK_TYPES[6];
                      return (
                  ...
                  <div className="text-center py-6 rounded-[14px]" style={{ border: "1.5px dashed var(--glass-border)", color: "var(--text-muted)" }}>
                    ...
                    <p className="text-[10px] font-mono mt-0.5" style={{ color: "var(--text-tertiary)" }}>Paste a URL above and click Add link</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex-shrink-0 px-5 pt-3 pb-4"
            style={{ borderTop: "0.5px solid var(--glass-border)", background: "var(--glass-fill-deep)", borderRadius: "0 0 24px 24px" }}>
            {/* Pagination dots */}
            ...
            <div className="flex gap-2">
              <button type="button" onClick={onClose} className="cc-btn py-3 text-[13px] font-medium flex-shrink-0" style={{ minWidth: "76px" }}>
                <span style={{ position: "relative", zIndex: 3 }}>Cancel</span>
              </button>

              {tab !== "basics" && (
                <button type="button" onClick={() => setTab(TABS[TABS.indexOf(tab)-1])}
                  className="cc-btn py-3 text-[13px] flex-shrink-0" style={{ minWidth: "64px" }}>
              ...
              {tab !== "resources" && canSubmit && (
                <button type="button" onClick={handleSubmit} disabled={loading}
                  className="flex-shrink-0 text-[10px] font-mono px-2" style={{ color: "var(--text-muted)" }}>
                  {loading ? "…" : "Quick"}
                </button>
              )}
            </div>
