-- ═══════════════════════════════════════════════════════════════════════
-- SRN Command Center — Learning v2 Migration
-- Run AFTER learning-progress-migration.sql (that table stays as-is).
-- Safe to run multiple times (idempotent).
-- ═══════════════════════════════════════════════════════════════════════

-- 1. Phase definitions table (replaces hardcoded PHASES constant)
CREATE TABLE IF NOT EXISTS learning_phases (
  ...
  -- JSON arrays stored as jsonb for easy querying
  -- resources: [{label, url}]
  resources     jsonb   NOT NULL DEFAULT '[]',
  -- tracks:   [{label, topics: [string]}]
  tracks        jsonb   NOT NULL DEFAULT '[]',
  -- weeks:    [{label, goals: [string]}]
  weeks         jsonb   NOT NULL DEFAULT '[]',
  -- practice: [{title, problems: [string]}]
...
-- 2. Week-level completion (separate from topic-level)
...
-- 5. Seed the 6 default phases (ON CONFLICT DO NOTHING — safe to re-run)
...
-- Reset serial sequence to avoid collision with seeded IDs
